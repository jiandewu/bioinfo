...
find ${PRJDIR} -name "*.script.sam" | parallel "java -jar ${PICARD} SortSam INPUT={} OUTPUT={.}.bam SORT_ORDER=coordinate"
# find ${PRJDIR} -type f -name "*.script.sam" -exec rm -f {} \;

find ${PRJDIR} -name "*.script.bam" -printf "%h\n" | uniq | parallel "samtools merge {}/sorted.bam {}/*.script.bam"
...
find ${PRJDIR} -name "dedup.bam" | parallel "java -jar ${GATK} -T RealignerTargetCreator -fixMisencodedQuals -R ${GE_REF} -I {} -o {//}/realignment_targets.list"
...
find ${PRJDIR} -name "raw_snps.vcf" | parallel "java -jar ${GATK} -T VariantFiltration -R ${GE_REF} -V {} --filterExpression 'QD < 2.0 || FS > 60.0 || MQ < 40.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0 || SOR > 4.0' --filterName 'basic_snp_filter' -o {//}/filtered_snps.vcf"
...
find ${PRJDIR} -name "filtered_*_final.vcf" | parallel "awk -F '\t' '${AWK_CMD}' {} > {.}_PASS.vcf"
...
find ${PRJDIR} -name "filtered_snps_final.vcf" | parallel "(cd {//} && java -jar ${SNPEFF} -v 'GRCh38.p7.RefSeq' {} > {//}/filtered_snps_final.ann.vcf)"
